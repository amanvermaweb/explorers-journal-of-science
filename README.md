...
- Contact form API
- Server-side persistence to JSON files in `data/`
...
- Stores submission in `data/contact-submissions.json`
...
- Stores subscriber in `data/newsletter-subscribers.json`
- Attempts notification email (best effort)

## Data and Email Notes

- Form and newsletter data are persisted to local JSON files in `data/`.
- Empty or missing data files are treated as empty arrays.
...
The current backend design is suitable for local/dev environments. Before production deployment, you should:

1. Replace local JSON persistence with a durable datastore (Postgres, MySQL, MongoDB, etc.).
